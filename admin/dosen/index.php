<?php
session_start();
include '../../config/koneksi.php';

// Cek Login
if (!isset($_SESSION['username'])) {
	header("location: ../../auth/login.php");
	exit();
}

// Cek level admin
if ($_SESSION['level'] != 'admin') {
	header("location: ../../index.php");
	exit();
}

$pesan = '';
$tipe_pesan = '';

// Tambah Data
if (isset($_POST['tambah'])) {
	$nidn = mysqli_real_escape_string($koneksi, $_POST['nidn']);
	$nama = mysqli_real_escape_string($koneksi, $_POST['nama']);

	// Cek apakah NIDN sudah ada
	$cek = mysqli_query($koneksi, "SELECT * FROM dosen WHERE nidn='$nidn'");
	if (mysqli_num_rows($cek) > 0) {
		$pesan = "NIDN sudah terdaftar!";
		$tipe_pesan = "danger";
	} else {
		$query = "INSERT INTO dosen (nidn, nama) VALUES ('$nidn', '$nama')";
		if (mysqli_query($koneksi, $query)) {
			$pesan = "Data dosen berhasil ditambahkan!";
			$tipe_pesan = "success";
		} else {
			$pesan = "Gagal menambahkan data: " . mysqli_error($koneksi);
			$tipe_pesan = "danger";
		}
	}
}

// Hapus Data
if (isset($_GET['hapus'])) {
	$nidn = mysqli_real_escape_string($koneksi, $_GET['hapus']);
	if (mysqli_query($koneksi, "DELETE FROM dosen WHERE nidn='$nidn'")) {
		$pesan = "Data dosen berhasil dihapus!";
		$tipe_pesan = "success";
	} else {
		$pesan = "Gagal menghapus data!";
		$tipe_pesan = "danger";
	}
}

// Update Data
if (isset($_POST['update'])) {
	$nidn_lama = mysqli_real_escape_string($koneksi, $_POST['nidn_lama']);
	$nidn = mysqli_real_escape_string($koneksi, $_POST['nidn']);
	$nama = mysqli_real_escape_string($koneksi, $_POST['nama']);

	$query = "UPDATE dosen SET nidn='$nidn', nama='$nama' WHERE nidn='$nidn_lama'";
	if (mysqli_query($koneksi, $query)) {
		$pesan = "Data dosen berhasil diupdate!";
		$tipe_pesan = "success";
	} else {
		$pesan = "Gagal mengupdate data: " . mysqli_error($koneksi);
		$tipe_pesan = "danger";
	}
}

?>
<!DOCTYPE html>
<html lang="en">

<head>
	<title>Data Dosen | SIA</title>
	<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
	<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.0/font/bootstrap-icons.css">
</head>

<body>
	<?php include '../../includes/sidebar.php'; ?>

	<div class="main-content">
		<div class="top-navbar">
			<h5 class="page-title">
				<i class="bi bi-person-badge-fill text-primary me-2"></i>Data Dosen
			</h5>
			<button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalTambah">
				<i class="bi bi-plus-circle me-1"></i> Tambah Dosen
			</button>
		</div>

		<div class="content-wrapper">
			<!-- Alert Pesan -->
			<?php if ($pesan != ''): ?>
				<div class="alert alert-<?= $tipe_pesan ?> alert-dismissible fade show" role="alert">
					<?= $pesan ?>
					<button type="button" class="btn-close" data-bs-dismiss="alert"></button>
				</div>
			<?php endif; ?>

			<!-- Card Tabel -->
			<div class="card shadow-sm">
				<div class="card-body">
					<div class="table-responsive">
						<table class="table table-striped table-bordered table-hover">
							<thead class="table-primary">
								<tr class="text-center">
									<th width="5%">No</th>
									<th width="20%">NIDN</th>
									<th width="50%">Nama Dosen</th>
									<th width="25%">Aksi</th>
								</tr>
							</thead>
							<tbody>
								<?php
								$no = 1;
								$data = mysqli_query($koneksi, "SELECT * FROM dosen ORDER BY nama ASC");

								if (mysqli_num_rows($data) > 0) {
									while ($row = mysqli_fetch_assoc($data)) {
										echo "<tr>
											<td class='text-center'>" . $no++ . "</td>
											<td><code>" . htmlspecialchars($row['nidn']) . "</code></td>
											<td>" . htmlspecialchars($row['nama']) . "</td>
											<td class='text-center'>
												<button class='btn btn-warning btn-sm me-1' data-bs-toggle='modal' data-bs-target='#modalEdit" . $row['nidn'] . "'>
													<i class='bi bi-pencil-square'></i> Edit
												</button>
												<a href='index.php?hapus=" . $row['nidn'] . "' class='btn btn-danger btn-sm' onclick=\"return confirm('Yakin ingin menghapus data " . htmlspecialchars($row['nama']) . "?')\">
													<i class='bi bi-trash'></i> Hapus
												</a>
											</td>
										</tr>";

										// Modal Edit
										echo '
										<div class="modal fade" id="modalEdit' . $row['nidn'] . '" tabindex="-1">
											<div class="modal-dialog">
												<div class="modal-content">
													<div class="modal-header bg-warning">
														<h5 class="modal-title">
															<i class="bi bi-pencil-square me-2"></i>Edit Data Dosen
														</h5>
														<button type="button" class="btn-close" data-bs-dismiss="modal"></button>
													</div>

													<form method="POST" action="index.php">
														<div class="modal-body">
															<input type="hidden" name="nidn_lama" value="' . $row['nidn'] . '">

															<div class="mb-3">
																<label class="form-label">NIDN</label>
																<input type="text" name="nidn" class="form-control" value="' . htmlspecialchars($row['nidn']) . '" maxlength="10" required>
															</div>

															<div class="mb-3">
																<label class="form-label">Nama Dosen</label>
																<input type="text" name="nama" class="form-control" value="' . htmlspecialchars($row['nama']) . '" required>
															</div>
														</div>

														<div class="modal-footer">
															<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
																<i class="bi bi-x-circle me-1"></i>Batal
															</button>
															<button type="submit" name="update" class="btn btn-primary">
																<i class="bi bi-check-circle me-1"></i>Simpan Perubahan
															</button>
														</div>
													</form>
												</div>
											</div>
										</div>';
									}
								} else {
									echo '<tr><td colspan="4" class="text-center text-muted">Belum ada data dosen</td></tr>';
								}
								?>
							</tbody>
						</table>
					</div>

					<!-- Info jumlah data -->
					<?php
					$total = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM dosen"));
					?>
					<div class="text-muted small">
						<i class="bi bi-info-circle me-1"></i>Total: <?= $total ?> dosen
					</div>
				</div>
			</div>
		</div>
	</div>

	<!-- Modal Tambah -->
	<div class="modal fade" id="modalTambah" tabindex="-1">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header bg-success text-white">
					<h5 class="modal-title">
						<i class="bi bi-plus-circle me-2"></i>Tambah Dosen
					</h5>
					<button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
				</div>

				<form method="POST" action="index.php">
					<div class="modal-body">
						<div class="mb-3">
							<label class="form-label">NIDN <span class="text-danger">*</span></label>
							<input type="text" name="nidn" class="form-control" placeholder="Masukkan NIDN (10 digit)" maxlength="10" required>
							<small class="text-muted">Nomor Induk Dosen Nasional</small>
						</div>

						<div class="mb-3">
							<label class="form-label">Nama Dosen <span class="text-danger">*</span></label>
							<input type="text" name="nama" class="form-control" placeholder="Masukkan Nama Lengkap" required>
						</div>
					</div>

					<div class="modal-footer">
						<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
							<i class="bi bi-x-circle me-1"></i>Batal
						</button>
						<button type="submit" name="tambah" class="btn btn-success">
							<i class="bi bi-check-circle me-1"></i>Simpan
						</button>
					</div>
				</form>
			</div>
		</div>
	</div>

	<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>

</body>

</html>